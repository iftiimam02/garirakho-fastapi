...
def on_startup():
    """
    Ensure tables exist.
    If DB is misconfigured, this will print the real error to Log Stream.
    """
    try:
        ...
        # For production, you usually WANT the app to fail if DB is down/misconfigured
        raise
...
def signup(
    ...
    if len(password) < 6:
        return templates.TemplateResponse(
            "signup.html", {"request": request, "error": "Password must be at least 6 characters"}
        )
