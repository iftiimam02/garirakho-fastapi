// /static/app.js
// Garirakho Dashboard JS (robust + Azure-friendly)
//
// Fixes:
// 1) Always sends cookies (credentials: "include") so session login works on Azure
// 2) Shows REAL error message instead of only "Failed to load devices"
// 3) Handles slots safely when DB returns JSON object (not array)
// 4) Optional: supports x-api-key if you set window.GARIRAKHO_API_KEY in template

function getApiHeaders() {
  const h = { Accept: "application/json" };

  // Optional support for API key (only if you expose it intentionally)
  // In your template you could set: <script>window.GARIRAKHO_API_KEY="...";</script>
  if (window.GARIRAKHO_API_KEY) {
    h["x-api-key"] = window.GARIRAKHO_API_KEY;
  }
  return h;
}
...
async function fetchDevices() {
  ...
  if (!r.ok) {
    ...
    throw new Error(`GET /api/devices -> HTTP ${r.status} | ${t}`);
  }
  return await r.json();
}
...
/**
 * Normalize slots into an array of:
 *   [{id:1, occupied:false, booked:false}, ...]
 *
 * Your DB currently may store JSON like:
 *   {"available":10,"occupied":2}
 * OR [] array.
 *
 * This keeps UI stable even without hardware.
 */
function normalizeSlots(slots) {
  // If already array, ensure each element has required keys
  ...
  // If it's an object like {"available":10,"occupied":2}
  if (slots && typeof slots === "object") {
    const available = Number(slots.available ?? 0);
    const occupied = Number(slots.occupied ?? 0);

    // If you know you have 4 slots physically, we can render 4
    const totalSlots = Math.max(4, available + occupied, 0);

    return Array.from({ length: totalSlots }, (_, i) => ({
      id: i + 1,
      occupied: i < occupied, // first N as occupied (demo)
      booked: false,
    }));
  }
    ...
    <div class="mt-4 border-t border-slate-800 pt-4">
      ...
      <div class="mt-2 flex flex-wrap gap-2">
        <button class="px-3 py-2 rounded-xl bg-amber-600 hover:bg-amber-500 text-sm"
          onclick="window.__garirakhoPost('/api/cmd/book-slots?deviceId=${encodeURIComponent(
            d.deviceId
          )}&slot1=true')">Book Slot 1</button>
        <button class="px-3 py-2 rounded-xl bg-amber-600 hover:bg-amber-500 text-sm"
          onclick="window.__garirakhoPost('/api/cmd/book-slots?deviceId=${encodeURIComponent(
            d.deviceId
          )}&slot2=true')">Book Slot 2</button>
        <button class="px-3 py-2 rounded-xl bg-amber-600 hover:bg-amber-500 text-sm"
          onclick="window.__garirakhoPost('/api/cmd/book-slots?deviceId=${encodeURIComponent(
            d.deviceId
          )}&slot3=true')">Book Slot 3</button>
        <button class="px-3 py-2 rounded-xl bg-amber-600 hover:bg-amber-500 text-sm"
          onclick="window.__garirakhoPost('/api/cmd/book-slots?deviceId=${encodeURIComponent(
            d.deviceId
          )}&slot4=true')">Book Slot 4</button>
...
// Expose post helper for inline onclick usage
